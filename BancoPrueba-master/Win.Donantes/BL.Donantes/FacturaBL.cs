...
namespace BL.Donantes
{
    public class FacturaBL
    {
        ...
        private Resultado Validar(Factura factura)
        {
            ...
            {
                resultado.Mensaje = "Agregue un Donante a la factura";
            }

            foreach (var detalle in factura.FacturaDetalle)
            {
                if (detalle.DonanteId == 0)
                {
                    resultado.Mensaje = "Seleccione un Donante Valido";
                    resultado.Exitoso = false;
                }
            }
        ...
        public void CalcularFactura(Factura factura)
        {
            if (factura != null)
            {
                ...
                foreach (var detalle in factura.FacturaDetalle)
                {
                    var donante = _contexto.Donantes.Find(detalle.DonanteId);
                    if (donante != null)
                    {
                        detalle.Precio = donante.Precio;
                        detalle.Total = detalle.Cantidad * donante.Precio;
